...
public class MainController {
    ...
    private Qubit excute(int num) {
        ...
        try {
            ...
            //System.gc();
    		//long startSpace = Runtime.getRuntime().freeMemory();
            ...
            //System.gc();
    		//long endSpace = Runtime.getRuntime().freeMemory();
    		
    		//System.out.println("对象内存:" + (startSpace - endSpace)/1000.0 + " KB");
...
//        }
    ...
    //增加内置的Ccnot矩阵
    @FXML
    private void addCcnot(ActionEvent event) throws IOException {
        if(getNodeId(event)!=null) {
            int okClicked = main.showCcnotDialog(getNodeId(event));
            addAndDraw(okClicked, "Ccnot");
        }
    }
    
    @FXML
    private void addCnots(ActionEvent event) throws IOException {
        if(getNodeId(event)!=null) {
            int okClicked = main.showCnotsDialog(getNodeId(event));
            addAndDraw(okClicked, "Cnots");
        }
    }
    ...
    private void addAndDraw(int okClicked, String type) {
        ...
        if ((okClicked == number - 1) && (okClicked != -1)) {
            ...
            } else if (type.equals("Ccnot")) {
                canvasManager.drawCcnotOperator(number - 2,
                        (CcnotOperator) circuit.getLastOperator());
            } else if (type.equals("Cnots")) {
                canvasManager.drawCnotsOperator(number - 2,
                        (CnotsOperator) circuit.getLastOperator());
